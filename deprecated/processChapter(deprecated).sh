...
while IFS= read -r name; do
    ...
    if jq 'has("chapters") and (.chapters != null and (.chapters | length > 0))' "$meta_json" | grep -q true; then
        ...
        for ((i = 0; i < $total_tracks; i++)); do
            ...
            chapter_song_target_dir="./${output_dir}/$(sanitize_metadata "$chapter_album")"
            chapter_song_target_file="$chapter_song_target_dir/$(sanitize_metadata "$chapter_title").opus"
            ...
            elif [[ ! -f "$chapter_song_target_file" ]]; then
